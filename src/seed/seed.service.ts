...
export class SeedService {
  ...
  async seed() {
    ...
    try {
      ...
      await this.seedInventories(20, products, warehouses);
      await this.seedReturns(5, orders);
      await this.seedPricings(20, products);
      await this.seedTransactions(10, orders);
  ...
  private async seedOrders(
    ...
    if (!products.length)
      throw new Error('No products available to assign to orders');
    if (!users.length)
      throw new Error('No users available to assign to orders');
